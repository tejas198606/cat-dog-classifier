...
# Load model
cnn_model = tf.keras.models.load_model(STATIC_FOLDER + "/models/" + "cat_dog_classifier1986.h5")   ## MODEL NAME 


IMAGE_SIZE = 224                                            ## Image Size can be changed 
...
def classify(model, image_path):
    ...
    label = "dog" if prob[0][0] >= 0.5 else "cat"                           ## need to rename the features names 
...
@app.route("/")
def home():
    return render_template("home111.html")                    ## HOME.HTML IS THE TEMPLATE NAME // it can be anything home11.html
...
def upload_file():
    ...
    return render_template(
        "classify111.html", image_file_name=file.filename, label=label, prob=prob) 
